...
- `source-commit` is the **consumer's** SHA (e.g. `dfe-receiver`'s
  commit), not the producer's (scalo-rs / scalo-py / hyperi-ci).
...
| ---------------------------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------- |
| Pre-push (on the image itself, at build time)              | `ghcr.io/hyperi-io/<app>:<tag>` (tag form)                           | Producer (scalo-rs / scalo-py / hyperi-ci)   |
...
flowchart LR
  ...
  C --> R[scalo-rs<br/>emit-artefacts]
  C --> P[scalo-py<br/>emit-artefacts]
