...
`hyperi-ci init` generates the files a consumer project needs: the
`.hyperi-ci.yaml` config, a `Makefile`, the `.github/workflows/ci.yml`
caller, a `CONTRIBUTING.md`, and `.githooks/` (commit-msg validation +
pre-push enforcement). It deliberately does NOT scaffold a `.releaserc`
-- tagging uses a central tagger-only semantic-release config supplied
...
"on":
  ...
  workflow_dispatch:
    inputs:
      ...
      from-head: { type: string, required: false, default: "" }
      bump: { type: string, required: false, default: "auto" }
jobs:
  ci:
