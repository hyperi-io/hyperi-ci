...
3. Generates `.hyperi-ci.yaml`, `Makefile`, `.github/workflows/ci.yml`,
   `.releaserc.yaml`, and `renovate.json`
4. Skips files that already exist (unless `--force`)
5. Detects deprecated config files (`.hypersec-ci.yaml`) and warns

## Dependency Management (Renovate)

Automated dependency updates are handled by [Mend Renovate](https://docs.renovatebot.com/)
(hosted GitHub App), replacing Dependabot.

### Architecture

```mermaid
flowchart LR
    subgraph Org["hyperi-io org"]
        preset["renovate-config repo\n(default.json — org preset)"]
        repo1["consumer repo\n(renovate.json)"]
        repo2["consumer repo\n(renovate.json)"]
    end

    mend["Mend Renovate App\n(hosted)"]
    mend -->|reads preset| preset
    mend -->|reads config, creates PRs| repo1
    mend -->|reads config, creates PRs| repo2
    repo1 -->|extends| preset
    repo2 -->|extends| preset
```

### How It Works

1. **Org preset** — [`hyperi-io/renovate-config`](https://github.com/hyperi-io/renovate-config)
   contains `default.json` with org-wide defaults (schedule, grouping,
   automerge rules, commit prefix)
2. **Per-repo config** — each repo has a minimal `renovate.json` that
   extends the org preset. Generated automatically by `hyperi-ci init`
3. **Mend hosted app** — runs Renovate on a schedule, creates PRs for
   dependency updates grouped by ecosystem

### Per-Repo Config

Generated by `hyperi-ci init`:
...
Repos can add overrides alongside the `extends` if needed.

### Org Preset Defaults

Defined in `hyperi-io/renovate-config/default.json`:

- Schedule: weekly
- Commit prefix: `chore(deps):` (no version bump via semantic-release)
- Automerge: patch and digest updates when CI passes
- Automerge: all GitHub Actions updates
- Grouping: by ecosystem (Rust, Python, TypeScript, Go, Docker, GitHub Actions)
...
[developer.mend.io/github/hyperi-io](https://developer.mend.io/github/hyperi-io)

Each repo also gets a "Dependency Dashboard" issue with update status.

### Why Renovate Over Dependabot

- Groups related updates into single PRs (Dependabot creates one PR per dep)
- Automerge with CI gate (Dependabot requires separate auto-merge config)
- Org-wide shared presets (Dependabot config is per-repo only)
- Supports lockfile maintenance (uv.lock, Cargo.lock, package-lock.json)
- SHA pinning for GitHub Actions out of the box
- Better scheduling and rate limiting controls
