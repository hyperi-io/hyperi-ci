...
**`quality.exclude_paths` takes names and paths.** A bare name (`data`) excludes every directory of that name at any depth. An entry with a `/` (`docs/generated`) is a path from the repo root and is dropped unless it is a directory. An entry that excludes nothing gets one warning per run.
